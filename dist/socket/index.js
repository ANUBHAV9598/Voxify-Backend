...
const emitPresenceUpdate = (io, payload, room) => {
    if (room) {
...
const isConversationMember = async (userId, conversationId) => {
...
const handlePresenceSync = async (io, socket, userId, payload) => {
    const requestedUserIds = Array.isArray(payload?.userIds) && payload.userIds.length > 0
...
export const registerSocketHandlers = (io) => {
    ...
    io.on("connection", async (socket) => {
        ...
        socket.on("conversation:join", async (payload) => {
            ...
            try {
                const isMember = await isConversationMember(userId, parsed.data.conversationId);
                if (!isMember) {
                    socket.emit("conversation:error", {
        ...
        socket.on("presence:sync", async (payload) => {
            try {
